...
const icons = [
  // Safety shield
  <svg key="safety" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" />
  </svg>,
  // Wifi/Connected
  <svg key="connected" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    <path strokeLinecap="round" strokeLinejoin="round" d="M8.288 15.038a5.25 5.25 0 0 1 7.424 0M5.106 11.856c3.807-3.808 9.98-3.808 13.788 0M1.924 8.674c5.565-5.565 14.587-5.565 20.152 0M12.53 18.22l-.53.53-.53-.53a.75.75 0 0 1 1.06 0Z" />
  </svg>,
  // Heart/Comfort
  <svg key="comfort" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
    <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
  </svg>,
  // Battery
  <svg key="battery" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
...
const FeaturesSection = ({ translations }: FeaturesSectionProps) => {
  ...
  return (
    <section id="features" className="py-20 small:py-28 bg-white">
      <div className="content-container">
        ...
        <div className="grid grid-cols-1 small:grid-cols-2 gap-6 small:gap-8">
          {items.map((feature, index) => (
            <div
              key={feature.title}
              className="group relative flex gap-5 p-6 small:p-8 rounded-2xl border border-dll-border hover:border-dll-foreground/20 bg-white hover:bg-dll-bg-secondary transition-all duration-300"
            >
              {/* Icon */}
              <div className="w-12 h-12 rounded-xl bg-dll-foreground flex items-center justify-center text-white shrink-0 group-hover:scale-110 transition-transform duration-300">
                {icons[index]}
              </div>
              {/* Text */}
              <div>
                <h3 className="text-base font-semibold text-dll-foreground mb-1.5">
                  {feature.title}
                </h3>
