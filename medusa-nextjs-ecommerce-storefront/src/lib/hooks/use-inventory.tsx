...
export function useInventoryCheck() {
  ...
  const checkInventory = useCallback(
    async (items: InventoryCheckItem[]) => {
      ...
      try {
        ...
        const response = await fetch(`${medusaUrl}/store/inventory`, {
          method: "POST",
        ...
        return data.inventory_check as InventoryCheckResult[]
      } catch (err) {
  ...
  return { checkInventory, isInStock, getAvailable, results, loading, error }
}

/**
 * 库存预留（购物车场景）
 * 在客户端管理临时库存预留，默认 15 分钟超时
 */
export function useInventoryReservation() {
  ...
  const reserve = useCallback(
    (variantId: string, quantity: number, timeoutMinutes: number = 15) => {
      ...
      setReserved((prev) => {
        ...
        next.set(variantId, { quantity, expiresAt })
        return next
      })
    },
    []
  ...
  return { reserve, release, getReserved, isExpired, reserved }
}
